...
All model intelligence in facilio-vision goes through four Studio agents on the
platform (org #2915, app `facilio-vision`). They are **prompt-configured**, not
trained: there is no fine-tuning API behind `facilio vibe agent`. "Tuning" here
means instructions, schemas, client-side validation, and a scored eval harness
that makes the quality a number instead of an opinion.

| agent          | id   | schema                      | consumed by |
| -------------- | ---- | --------------------------- | ----------- |
| `fv-identify`  | 6262 | `agents/identify.schema.json`  | `identifyAsset()` → `src/voice/reportFault.ts` |
| `fv-wo-draft`  | 6263 | `agents/wo-draft.schema.json`  | `draftWorkOrder()` → `reportFault.ts`, `ARScreen` |
| `fv-nameplate` | 6264 | `agents/nameplate.schema.json` | `readNameplate()` |
| `fv-voice`     | 6265 | **none, deliberately**      | `voiceTurn()` → `src/voice/toolLoop.ts` |

Live model: `openai / gpt-5.5` on all four (not claude — check with
