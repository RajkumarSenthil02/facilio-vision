...
export const CASES = [
  ...
  },
  {
    ...
    expect(raw) {
      ...
      if (keys !== 'description,priority,subject') return `unexpected keys: ${keys}`;
      if (!['High', 'Medium', 'Low'].includes(d.priority))
        return `priority outside enum: ${d.priority}`;
      if (/4471/.test(`${d.subject} ${d.description}`)) return 'echoed the unverified work order number';
      return true;
    },
  },
  ...
  },
  {
    ...
    why: 'Only the six listed tools exist; a request near-miss must not mint a seventh.',
